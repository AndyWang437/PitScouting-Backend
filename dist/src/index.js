...
const direct_fix_1 = __importDefault(require("./routes/direct-fix"));
...
const checkDatabaseTables = async () => {
    try {
        ...
        for (const table of requiredTables) {
            try {
                const [result] = await init_1.sequelize.query(`SELECT COUNT(*) as count FROM ${table}`);
...
async function createTablesDirectly() {
    ...
    try {
        if (init_1.sequelize.getDialect() === 'postgres') {
