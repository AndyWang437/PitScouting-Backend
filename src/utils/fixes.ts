 ...
 */
export const debugTeamData = (teamData: any, label = 'Team Data') => {
  if (!teamData) {
 ...
 */
export const fixBooleanValues = (teamData: any) => {
 ...
 */
export const fixCoralLevels = (teamData: any) => {
  if (!teamData) return teamData;
 ...
 */
export const applyFrontendFixes = () => {
  ...
  document.querySelectorAll('[data-image-path]').forEach(element => {
    ...
    const fixedPath = fixImagePath(path);
